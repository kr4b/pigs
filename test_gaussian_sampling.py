...
start = time.time()
result1 = gaussians.sample_gaussians(means, conics, opacities, values, samples)
img1 = result1.reshape(res, res).detach().cpu().numpy()
print("Original:", time.time() - start)

sampler = GaussianSampler(True)

start = time.time()
result2, radii = sampler(
...
img2 = result2.reshape(res, res).detach().cpu().numpy()
