...
covariances = covariances.reshape(-1, d, d)#.to(torch.float64)
conics = conics.reshape(-1, d, d)#.to(torch.float64)
opacities = opacities.reshape(-1)#.to(torch.float64)
...
grad_x_auto = torch.autograd.grad(result_py.sum(), (means, values, conics, opacities, samples), retain_graph=True, create_graph=True)
grad_x_cuda_auto = torch.autograd.grad(result_cuda.sum(), (means, values, conics, opacities, samples), retain_graph=True, create_graph=True)
...
img_py = gaussians.gaussian_derivative(means, conics, opacities, values, samples)
...
grad_xx_auto = torch.autograd.grad(grad_x_auto[...,0].sum(), (means, values, conics, opacities, samples), retain_graph=True, create_graph=True)
grad_yy_auto = torch.autograd.grad(grad_x_auto[...,1].sum(), (means, values, conics, opacities, samples), retain_graph=True, create_graph=True)
lap_auto = []
for i in range(len(grad_xx_auto)):
    lap_auto.append(torch.cat((grad_xx_auto[i].unsqueeze(-1), grad_yy_auto[i].unsqueeze(-1)), dim=-1))

grad_xx_cuda_auto = torch.autograd.grad(grad_x_cuda[...,0].sum(), (means, values, conics, opacities, samples), retain_graph=True)
grad_yy_cuda_auto = torch.autograd.grad(grad_x_cuda[...,1].sum(), (means, values, conics, opacities, samples))
lap_cuda_auto = []
...
img_py = gaussians.gaussian_derivative2(means, conics, opacities, values, samples)
img_py = img_py.reshape(res, res, -1).detach().cpu().numpy()
...
grad_xx_auto = torch.autograd.grad(lap_auto[...,0,0].sum(), (means, values, conics, opacities, samples), retain_graph=True)
grad_xy_auto = torch.autograd.grad(lap_auto[...,0,1].sum(), (means, values, conics, opacities, samples), retain_graph=True)
grad_yx_auto = torch.autograd.grad(lap_auto[...,1,0].sum(), (means, values, conics, opacities, samples), retain_graph=True)
grad_yy_auto = torch.autograd.grad(lap_auto[...,1,1].sum(), (means, values, conics, opacities, samples))
...
grad_xx_cuda_auto = torch.autograd.grad(lap_cuda[...,0,0].sum(), (means, values, conics, opacities, samples), retain_graph=True)
grad_xy_cuda_auto = torch.autograd.grad(lap_cuda[...,0,1].sum(), (means, values, conics, opacities, samples), retain_graph=True)
grad_yx_cuda_auto = torch.autograd.grad(lap_cuda[...,1,0].sum(), (means, values, conics, opacities, samples), retain_graph=True)
grad_yy_cuda_auto = torch.autograd.grad(lap_cuda[...,1,1].sum(), (means, values, conics, opacities, samples))
